...
func make_rooms():
	...
	for room in $Rooms.get_children():   
		if randf() < cull:
			...
			room.mode = RigidBody2D.MODE_STATIC     #stop movement
			room_positions.append(Vector3(room.position.x, room.position.y, 0))   #add room to room list
...
func _draw():
	if start_room:
		draw_string(font, start_room.position-Vector2(125,0), "start", Color(1,1,1))  #label start room
	if end_room:
		draw_string(font, end_room.position-Vector2(125,0), "end", Color(1,1,1))      #label end room
	if play_mode:
		return
	for room in $Rooms.get_children():
...
func _input(event):    #re-reandomiz rooms when spacebar is hit
	if event.is_action_pressed('ui_select'):    #spacebar changes map generation
		if play_mode:
			player.queue_free()
			play_mode = false
		...
		end_room = null
		make_rooms()
	if event.is_action_pressed('ui_focus_next'): #tab button to generate tiles
		make_map()
	if event.is_action_pressed('ui_cancel'):     #escape button to start play mode
...
func find_mst(nodes):
	...
	#repeat untl no more nodes remain
	while nodes:
		...
		for p1 in path.get_points():
			...
			#loop through the remaining nodes in array
			for p2 in nodes:
...
func make_map():
	#Create a TileMap from the generate rooms and path
	...
	for room in $Rooms.get_children():
		...
		var ul = (room.position / tile_size).floor() - s   #room.position is center of room
